...
global next_move
next_move=""
global initial_iteration_check
initial_iteration_check=1
...
class ComputerPlayer:
    ...
    def get_moves(self, quintris):
        ...
        global next_move
        moves= self.successor_boards(next_move)
        current_move=moves.split("-_-")[0]
        next_move = moves.split("-_-")[1]
        return current_move
    ...
    def evaluation_function(self,board,type_val):
        ...
        #Weighted_row_coverage_and_lessgaps : Will try to fill the bottom rows first and with less gaps
            ...
            return -100*height+gaps+val-1000*line_filled+column_gap
    ...
    def successor_boards(self,next_move):
        ...
        # and pull it down. 
        global initial_iteration_check
        if (initial_iteration_check):
            initial_iteration_check=0
            ...
            quintris_new=copy.deepcopy(quintris)
            for i in next_move:
                if i=="n":
                    quintris_new.rotate()
                elif i=="b":
            ...
            for rotate_angle in rotation:
                #Creating a copy of the board object so that we can use the methods of rotate and flip from backend codes provided by the Professor and without changing the original board
                quintris_game=copy.deepcopy(quintris_new)
                ...
                for left in range(0,quintris_game.get_piece()[2]+1):
                    ...
                    move_tracker[next_move+"-_-"+rotate_move+"b"*len(left_tracker)] = (quintris_game_copy.get_board(),self.evaluation_function(quintris_game_copy.get_board(),"Weighted_row_coverage_and_lessgaps"))
